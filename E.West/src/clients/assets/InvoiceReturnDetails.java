...
public class InvoiceReturnDetails {
    ...
    public InvoiceReturnDetails(int id, String product, String amount, String cost, String totalcost) {
        this.id = id;
    ...
    public InvoiceReturnDetails(int id, ObservableList<StoreProducts> data, String selectedpro, String amount, String cost, String totalCostString) {
        this.id = id;
    ...
    public static ObservableList<InvoiceReturnDetails> getData(int id, int invoiceId) throws Exception {
        ObservableList<InvoiceReturnDetails> data = FXCollections.observableArrayList();
        ...
        while (rs.next()) {
            data.add(new InvoiceReturnDetails(rs.getInt(1), data1, rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
        }
        return data;
    }

    public static ObservableList<InvoiceReturnDetails> getData(int id, ObservableList<StoreProducts> invoiceId) throws Exception {
        ObservableList<InvoiceReturnDetails> data = FXCollections.observableArrayList();
        ResultSet rs = db.get.getReportCon().createStatement().executeQuery("SELECT `cli_returned_invoice_details`.`id`,`st_products`.`name`, `cli_returned_invoice_details`.`amount`, `cli_returned_invoice_details`.`cost`,`cli_returned_invoice_details`.`total_cost` FROM `cli_returned_invoice_details`,`st_products` WHERE `st_products`.`id` =`cli_returned_invoice_details`.`product_id` AND `cli_returned_invoice_details`.`returned_id`='" + id + "'");
        while (rs.next()) {
            data.add(new InvoiceReturnDetails(rs.getInt(1), invoiceId, rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
        }
        return data;
    }

    public static ObservableList<InvoiceReturnDetails> getDataForExite(int id) throws Exception {
        ObservableList<InvoiceReturnDetails> data = FXCollections.observableArrayList();
        ObservableList<StoreProducts> data1 = StoreProducts.getDataOfInvoice(id);
        ResultSet rs = db.get.getReportCon().createStatement().executeQuery("SELECT `cli_returned_invoice_details`.`id`,`st_products`.`name`, `cli_returned_invoice_details`.`amount`, `cli_returned_invoice_details`.`cost`,`cli_returned_invoice_details`.`total_cost` FROM `cli_returned_invoice_details`,`st_products` WHERE `st_products`.`id` =`cli_returned_invoice_details`.`product_id` AND `cli_returned_invoice_details`.`returned_id`='" + id + "'");
        while (rs.next()) {
            data.add(new InvoiceReturnDetails(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
        }
        return data;
    }
}
