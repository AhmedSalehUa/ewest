...
public class TrackSellDetails {
    ...
    public TrackSellDetails(int id, String product, String amount, String cost, String totalcost) {
        this.id = id;
    ...
    public TrackSellDetails(int id, ObservableList<StoreProducts> data, String selectedpro, String amount, String cost, String totalCostString) {
        this.id = id;
    ...
    public static ObservableList<TrackSellDetails> getData(int id, int invoiceId) throws Exception {
        ObservableList<TrackSellDetails> data = FXCollections.observableArrayList();
        System.out.println(id);
        ObservableList<StoreProducts> data1 = StoreProducts.getData();
        ResultSet rs = db.get.getReportCon().createStatement().executeQuery("SELECT `cli_invoices_details`.`id`,`st_products`.`name`, `cli_invoices_details`.`amount`, `cli_invoices_details`.`cost`,`cli_invoices_details`.`total_cost` FROM `cli_invoices_details`,`st_products` WHERE `st_products`.`id` =`cli_invoices_details`.`product_id` AND `st_returned_invoice_details`.`invoice_id`='" + id + "'");
        while (rs.next()) {
            data.add(new TrackSellDetails(rs.getInt(1), data1, rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
        }
        return data;
    }
    public static ObservableList<TrackSellDetails> getDataForExite(int id) throws Exception {
        ObservableList<TrackSellDetails> data = FXCollections.observableArrayList(); 
        ...
        while (rs.next()) {
            data.add(new TrackSellDetails(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
        }
        return data;
    }
    public static ObservableList<TrackSellDetails> getData(int id, ObservableList<StoreProducts> invoiceId) throws Exception {
        ObservableList<TrackSellDetails> data = FXCollections.observableArrayList();
        ResultSet rs = db.get.getReportCon().createStatement().executeQuery("SELECT `sl_out_invoice_details`.`id`,`st_products`.`name`, `sl_out_invoice_details`.`amount`, `sl_out_invoice_details`.`cost`,`sl_out_invoice_details`.`total_cost` FROM `sl_out_invoice_details`,`st_products`,`st_store_products` WHERE `sl_out_invoice_details`.`store_product_id`=`st_store_products`.`id` AND `st_products`.`id` =`st_store_products`.`product_id` AND `sl_out_invoice_details`.`invoice_id`='"+id+"'");
        while (rs.next()) {
            data.add(new TrackSellDetails(rs.getInt(1), invoiceId, rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
        }
        return data;
    }
}
